                              ...
                              const struct boot_info *bi)
{
    ...
    /*
     * XXX: this loop uses transitionary format of struct boot_module, added in
     * 7cf6e073e47 "x86/boot: introduce struct boot_module", and will have to
     * be updated if that definition changes.
     */
...
void tpm_process_drtm_policy(const struct boot_info *bi)
{
    ...
    check_drtm_policy(slrt, policy, policy_entry, bi);
