...
static void smp_callin(void)
{
    ...
    while ( cpu_state[cpu] != CPU_STATE_ONLINE )
        cpu_relax();
}
...
void asmlinkage start_secondary(unsigned int cpu)
{
    ...
    while ( cpu_state[cpu] != CPU_STATE_CALLOUT )
        cpu_relax();
...
static int do_boot_cpu(int apicid, int cpu)
{
    ...
    if ( !boot_error )
    {
        ...
        if ( cpu_state[cpu] == CPU_STATE_INIT )
        {
            ...
            {
                if ( cpu_state[cpu] != CPU_STATE_CALLOUT )
                    break;
                udelay(10);
            }
        }

        if ( cpu_state[cpu] == CPU_STATE_CALLIN )
        {
        ...
        else if ( cpu_state[cpu] == CPU_STATE_DEAD )
        {
            smp_rmb();
            rc = cpu_error;
        }
...
void __cpu_die(unsigned int cpu)
{
    ...
    while ( (seen_state = cpu_state[cpu]) != CPU_STATE_DEAD )
    {
